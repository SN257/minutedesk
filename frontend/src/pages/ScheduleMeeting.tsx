  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-white border-b-4 border-slate-600 rounded-2xl shadow-sm p-4 sm:p-6 mb-4 sm:mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Schedule Meeting</h2>
          <p className="text-sm sm:text-base text-gray-600 mt-1">Pick a date on the calendar and finalize booking in the popup.</p>
        </div>
        <div />
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-7 gap-2 mb-4">
              {weekDays.map(w => (
                <div key={w} className="text-center text-xs font-bold text-slate-600 uppercase tracking-wider py-3">
                  {w}
                </div>
              ))}
            </div>
                  ...
                  <div key={idx} className="relative group/tooltip">
                    <button
                      ...
                      className={`
                          ...
                          ${isOtherMonth
                            ? 'bg-white text-gray-400 border-transparent opacity-70 cursor-default'
                            : past
                              ? 'bg-gray-50 text-gray-300 cursor-not-allowed'
                              : selected
                                ? 'bg-slate-700 text-white shadow-xl scale-105 ring-2 ring-slate-400 ring-offset-2'
                                : today
                                  ? 'bg-slate-100 text-slate-800 border-2 border-slate-300 hover:bg-slate-200 hover:shadow-lg hover:scale-105'
                                  : 'bg-white border border-gray-200 hover:border-slate-400 hover:shadow-lg hover:scale-105 text-gray-700'
                        }
                        `}
                    >
                      {/* Date Number */}
                      <span className={`text-lg font-bold ${selected ? 'text-white' : today ? 'text-slate-800' : isOtherMonth ? 'text-gray-400' : ''}`}>
                        {date.getDate()}
                      </span>

                      {/* Meeting count */}
                      {hasScheduledMeetings && !isOtherMonth && (
                        <span className={`text-[10px] font-medium mt-0.5 ${selected ? 'text-white/90' : 'text-slate-600'}`}>
                      ...
                      {/* Today Badge */}
                      {today && !selected && (
                        <div className="absolute -top-1 -right-1 w-2 h-2 bg-slate-700 rounded-full"></div>
                      )}
                    </button>
      ...
      {selectedDate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 animate-fadeIn">
          ...
          <div className="relative z-20 w-full max-w-6xl bg-white rounded-2xl shadow-2xl overflow-hidden animate-slideUp">
            <div className="p-0 flex flex-col" style={{ maxHeight: '92vh' }}>
              {/* Modal Header with Gradient */}
                ...
                <div className="p-6 lg:p-10 bg-gradient-to-br from-gray-50 to-white flex-1">
                  <div className="flex flex-col h-full">
                    <div className={`flex-1 overflow-y-auto pr-2`} style={modalStep === 2 ? { maxHeight: 'calc(92vh - 220px)' } : modalStep === 1 ? { maxHeight: 'calc(92vh - 220px)' } : undefined}>
                        ...
                        <div className="space-y-6">
                          <div className="flex items-center justify-between mb-4">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">Select your preferred meeting time</p>
                            </div>
                            <div className="flex items-center gap-3">
                              ...
                              <button
                                ...
                                className={`relative p-4 rounded-xl border-2 transition-all duration-200 ${!slot.available
                                    ...
                                    : selectedTimeSlot?.id === slot.id
                                      ? 'bg-gradient-to-br from-slate-800 to-slate-900 border-slate-800 shadow-lg scale-105'
                                      : 'bg-white border-gray-200 hover:border-slate-400 hover:shadow-md hover:scale-102'
                                  }`}
                              >
                                <div className={`text-center ${selectedTimeSlot?.id === slot.id ? 'text-white' : 'text-slate-800'}`}>
                                  <div className="flex items-center justify-center mb-1">
                                    <svg className={`w-4 h-4 mr-1 ${selectedTimeSlot?.id === slot.id ? 'text-white' : 'text-slate-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <span className="text-lg font-bold">{slot.time}</span>
                                  </div>
                                  <div className={`text-xs ${selectedTimeSlot?.id === slot.id ? 'text-white/80' : 'text-gray-500'}`}>
                          ...
                          {selectedTimeSlot && (
                            <div className="bg-gradient-to-r from-slate-50 to-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="text-sm font-semibold text-slate-800">Selected: {selectedTimeSlot.time}</div>
                                  <div className="text-xs text-gray-600">{selectedDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</div>
                                </div>
                              </div>
                      ...
                      {modalStep === 2 && (
                        <div className="space-y-6">
                          <div className="bg-gradient-to-r from-slate-100 to-slate-50 border-l-4 border-slate-700 rounded-xl p-5 flex items-center gap-4">
                            ...
                            <div className="flex-1">
                              <div className="text-sm text-gray-600">Your selected time slot</div>
                              <div className="text-2xl font-bold text-slate-800">{selectedTimeSlot?.time}</div>
                              <div className="text-xs text-gray-500 mt-1">Ends at {selectedTimeSlot ? calculateEndTime(selectedTimeSlot.time, duration) : '-'} ({formatDurationLabel(duration)})</div>
                            </div>
                          </div>
                            ...
                            <div className="space-y-4 lg:col-span-1">
                              <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                                <div className="flex items-center gap-2 mb-4">
                                  ...
                                  <h4 className="text-sm font-semibold text-slate-800">Meeting Type</h4>
                                </div>
                                <div>
                              ...
                              <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                                <div className="flex items-center gap-2 mb-4">
                                  ...
                                  <h4 className="text-sm font-semibold text-slate-800">Center</h4>
                                </div>
                                <div>
                                  <div className="relative" ref={centerDropdownRef}>
                                    <input
                                      ...
                                      className="w-full bg-white border-2 border-gray-300 rounded-lg px-4 py-3 pr-10 text-gray-900 font-medium placeholder-gray-400 focus:outline-none focus:border-slate-600 focus:ring-2 focus:ring-slate-600 focus:ring-opacity-20 transition-colors cursor-text"
                                    />
                                    <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-500">
                                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                            ...
                            <div className="space-y-4 lg:col-span-1">
                              <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                                <div className="flex items-center gap-2 mb-4">
                                  ...
                                  <h4 className="text-sm font-semibold text-slate-800">Duration</h4>
                                </div>
                                <div className="relative" ref={durationListContainerRef}>
                                  <button
                                    ...
                                    className="w-full bg-white border-2 border-gray-300 rounded-lg px-4 py-3 pr-10 text-gray-900 font-medium focus:outline-none focus:border-slate-600 focus:ring-2 focus:ring-slate-600 focus:ring-opacity-20 transition-colors cursor-pointer hover:bg-gray-50 text-left"
                                  >
                                    <span className="text-gray-900">{formatDurationLabel(duration)}</span>
                                  </button>
                                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-500">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                                  ...
                                  {showDurationDropdown && (
                                    <div id="duration-listbox" role="listbox" className="absolute z-20 w-full mt-2 bg-white border-2 border-gray-300 rounded-lg shadow-lg max-h-60 overflow-auto">
                                      {durations.map((d, idx) => (
                                        <div
                                          key={d}
                                          role="option"
                                          ...
                                          className={`w-full text-left px-4 py-3 cursor-pointer ${idx === durationHighlightedIndex ? 'bg-slate-200 text-slate-900 font-semibold' : 'hover:bg-slate-100 text-gray-900'}`}
                              ...
                              <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                                ...
                                <input
                                  ...
                                  min={1}
                                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg text-sm focus:border-slate-500 focus:outline-none transition-colors"
                                />
                              </div>
                          ...
                          <div className="mt-4 w-full">
                            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm w-full">
                              <div className="flex items-center gap-2 mb-4">
                                ...
                                <h4 className="text-sm font-semibold text-slate-800">Meeting Agenda</h4>
                              </div>
                              <div className="space-y-3 max-h-56 overflow-y-auto">
                                {points.map((p, i) => (
                                  <div key={p.id} className="flex items-center gap-3 bg-gray-50 rounded-lg p-2">
                                    <div className="flex-shrink-0 w-6 h-6 bg-slate-700 text-white rounded-full flex items-center justify-center text-xs font-bold">{i + 1}</div>
                                    <input
                                      value={p.text}
                                      onChange={(e) => updatePoint(p.id, e.target.value)}
                                      placeholder={`Agenda point ${i + 1}`}
                                      className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-slate-500 focus:outline-none"
                                    />
                                    {points.length > 1 && (
                                      <button onClick={() => removePoint(p.id)} className="flex-shrink-0 text-red-500 hover:text-red-700 hover:bg-red-50 p-2 rounded-lg transition-colors">
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              ...
                              <button onClick={addPoint} className="mt-3 w-full py-2 text-sm text-slate-700 hover:text-slate-900 border-2 border-dashed border-gray-300 hover:border-slate-400 rounded-lg transition-colors flex items-center justify-center gap-2">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {modalStep === 2 && (
                      <div className="flex items-center gap-3 pt-4 border-t">
                          ...
                          <button onClick={() => setModalStep(1)} className="px-5 py-3 border-2 border-gray-300 text-slate-700 rounded-lg hover:bg-gray-50 transition-all font-medium flex items-center gap-2">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                            </svg>
                            Back
                          </button>
                          <button onClick={() => setModalStep(3)} disabled={!selectedTimeSlot} className="px-5 py-3 bg-slate-700 text-white rounded-lg hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md font-medium flex items-center justify-center gap-2">
                            Continue
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                            </svg>
                    ...
                    {modalStep === 3 && (
                      <div className="space-y-6">
                        ...
                        <div className="bg-gradient-to-br from-slate-50 to-gray-50 rounded-xl p-6 border border-gray-200 w-full">
                          <h4 className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-3">Meeting Summary</h4>
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="space-y-2 text-sm">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium text-slate-800">{selectedTimeSlot?.time} - {selectedTimeSlot ? calculateEndTime(selectedTimeSlot.time, duration) : '-'}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">Duration:</span>
                                <span className="font-medium text-slate-800">{formatDurationLabel(duration)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">Meeting Type:</span>
                                <span className="font-medium text-slate-800">{meetingType || '-'}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">Center:</span>
                                <span className="font-medium text-slate-800">{center || '-'}</span>
                              </div>
                              <div className="flex justify-between">
                            ...
                            <div className="lg:pl-6 lg:border-l lg:border-gray-200">
                              <h4 className="text-sm font-semibold text-slate-800 mb-2">Agenda</h4>
                        ...
                        <div className="flex items-center gap-3 pt-4 border-t">
                          <div className="ml-auto flex items-center gap-3">
                            <button onClick={() => setModalStep(2)} className="px-5 py-3 border-2 border-gray-300 text-slate-700 rounded-lg hover:bg-gray-50 transition-all font-medium flex items-center gap-2">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                              </svg>
                              Back
