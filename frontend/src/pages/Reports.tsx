    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 flex items-center justify-center">
        <div className="text-center">
          <div className="w-14 h-14 border-4 border-slate-200 border-t-slate-900 rounded-full animate-spin mx-auto mb-4"></div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200 -m-6 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white shadow-sm rounded-2xl p-4 md:p-6 mb-4 md:mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-4 border-b-4 border-slate-700 animate-slideDown relative z-20">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Reports & Analytics</h2>
            <p className="text-sm sm:text-base text-slate-600 mt-1 font-medium">Track your meetings and tasks</p>
          </div>

          <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto">
            <button
              onClick={exportToCSV}
              className="px-5 py-2.5 bg-gradient-to-r from-slate-800 to-slate-900 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200"
            >
        ...
        {/* Modern Tab System - Full Width to Match Header/Table */}
        <div className="max-w-7xl mx-auto">
          <div className="flex gap-3 bg-white rounded-2xl p-2 shadow-lg">
            <button
              onClick={() => setActiveTab('meetings')}
              className={`relative flex-1 px-8 py-4 rounded-xl text-base font-bold transition-all duration-300 ${activeTab === 'meetings'
            ...
            <button
              ...
              className={`relative flex-1 px-8 py-4 rounded-xl text-base font-bold transition-all duration-300 ${activeTab === 'tasks'
                ? 'bg-gradient-to-r from-slate-900 to-slate-800 text-white shadow-lg'
                : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'
                }`}
            >
        ...
        <div className="bg-white rounded-2xl shadow-xl">
          ...
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-4 rounded-t-2xl">
            <div className="flex items-center justify-between">
              ...
              <h2 className="text-lg font-black text-white">
                {activeTab === 'meetings'
                  ? `${activeReport === 'lessons' ? 'Lessons' : activeReport === 'decisions' ? 'Decisions' : 'Information'} Report`
                  : 'Tasks Report'}
              </h2>
              ...
              {activeTab === 'meetings' ? (
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="flex items-center gap-3 px-4 py-2.5 bg-white hover:bg-slate-50 rounded-xl transition-all shadow-lg"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 bg-slate-900 text-white rounded-full text-xs font-black">
                        {activeReport === 'lessons' ? allLessons.length : activeReport === 'decisions' ? allDecisions.length : allInformation.length}
                      </span>
                    </div>
                    <svg className={`w-4 h-4 text-slate-900 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {/* Dropdown Menu */}
                  {dropdownOpen && (
                    <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden z-50">
                      <button
                        onClick={() => { setActiveReport('lessons'); setDropdownOpen(false); }}
                        className={`w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors ${activeReport === 'lessons' ? 'bg-slate-100' : ''}`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${activeReport === 'lessons' ? 'bg-slate-900' : 'bg-slate-100'}`}>
                            <svg className={`w-4 h-4 ${activeReport === 'lessons' ? 'text-white' : 'text-slate-700'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                            </svg>
                          </div>
                          <span className="text-sm font-bold text-slate-900">Lessons Learned</span>
                        </div>
                        <span className="px-2 py-1 bg-slate-900 text-white rounded-full text-xs font-bold">{allLessons.length}</span>
                      </button>

                      <button
                        onClick={() => { setActiveReport('decisions'); setDropdownOpen(false); }}
                        className={`w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors ${activeReport === 'decisions' ? 'bg-slate-100' : ''}`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${activeReport === 'decisions' ? 'bg-slate-900' : 'bg-slate-100'}`}>
                            <svg className={`w-4 h-4 ${activeReport === 'decisions' ? 'text-white' : 'text-slate-700'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                          </div>
                          <span className="text-sm font-bold text-slate-900">Decisions Made</span>
                        </div>
                        <span className="px-2 py-1 bg-slate-900 text-white rounded-full text-xs font-bold">{allDecisions.length}</span>
                      </button>

                      <button
                        onClick={() => { setActiveReport('information'); setDropdownOpen(false); }}
                        className={`w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors ${activeReport === 'information' ? 'bg-slate-100' : ''}`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${activeReport === 'information' ? 'bg-slate-900' : 'bg-slate-100'}`}>
                            <svg className={`w-4 h-4 ${activeReport === 'information' ? 'text-white' : 'text-slate-700'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                          </div>
                          <span className="text-sm font-bold text-slate-900">Information Items</span>
                        </div>
                        <span className="px-2 py-1 bg-slate-900 text-white rounded-full text-xs font-bold">{allInformation.length}</span>
                      </button>
                    </div>
                  )}
                </div>
          ...
          <div className="rounded-2xl shadow-xl border border-slate-200 bg-white">
            ...
            <div className="bg-white min-h-[600px] rounded-2xl relative pb-32">
              {currentData.length === 0 ? (
                ...
                <table className="w-full border-collapse !overflow-visible">
                  ...
                  <thead className="bg-gradient-to-r from-slate-50 to-slate-100 sticky top-0 z-50 !overflow-visible">
                    <tr className="border-b-2 border-slate-200 !overflow-visible">
                      {activeTab === 'meetings' && (activeReport === 'lessons' || activeReport === 'decisions') && (
                        <th className="px-6 py-4 text-left bg-slate-50 !overflow-visible relative">
                          <div className="flex items-center justify-between group relative">
                            <button
                              ...
                              className="flex items-center gap-2 text-xs font-extrabold text-slate-900 uppercase tracking-wider hover:text-slate-700 transition-colors"
                            >
                              Status
                              <svg className={`w-4 h-4 transition-all ${sortConfig?.column === 'isCompleted' ? 'text-slate-900' : 'text-slate-400 group-hover:text-slate-600'} ${sortConfig?.column === 'isCompleted' && sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div className="relative">
                              ...
                              {columnMenuOpen === 'status' && (
                                <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px] -ml-1">
                                  ...
                                  <button onClick={() => { setFilters({ ...filters, status: '' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 ${filters.status === '' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>All</button>
                                  <button onClick={() => { setFilters({ ...filters, status: 'completed' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 ${filters.status === 'completed' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>Completed</button>
                                  <button onClick={() => { setFilters({ ...filters, status: 'pending' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 ${filters.status === 'pending' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>Pending</button>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      <th className="px-6 py-4 text-left bg-slate-50/50">
                        <div className="flex items-center justify-between group relative">
                          <button
                            onClick={() => setSortConfig({
                          ...
                          >
                            ...
                            <svg className={`w-4 h-4 transition-all ${sortConfig?.column === (activeTab === 'meetings' ? 'text' : 'title') ? 'text-slate-900' : 'text-slate-400 group-hover:text-slate-600'} ${sortConfig?.column === (activeTab === 'meetings' ? 'text' : 'title') && sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 11l5-5m0 0l5 5m-5-5v12" />
                            </svg>
                          </button>
                          <div className="relative">
                            ...
                            {columnMenuOpen === 'content' && (
                              <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                <div className="px-4 py-2 border-b border-slate-100 mb-1 flex items-center justify-between">
                                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Options</span>
                                  <button onClick={() => setColumnMenuOpen(null)} className="text-slate-400 hover:text-slate-600">
                                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                                  </button>
                                </div>
                                <button onClick={() => { setSortConfig({ column: activeTab === 'meetings' ? 'text' : 'title', direction: 'asc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 text-slate-600">Sort by ASC</button>
                                <button onClick={() => { setSortConfig({ column: activeTab === 'meetings' ? 'text' : 'title', direction: 'desc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 text-slate-600">Sort by DESC</button>
                              </div>
                            )}
                          </div>
                        </div>
                          ...
                          <div className="flex items-center justify-between group relative">
                            ...
                            <div className="relative">
                              ...
                              {columnMenuOpen === 'category' && (
                                <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                  <div className="px-4 py-2 border-b border-slate-100 mb-1 flex items-center justify-between">
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Filter Category</span>
                                    <button onClick={() => setColumnMenuOpen(null)} className="text-slate-400 hover:text-slate-600">
                                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                                    </button>
                                  </div>
                                  <input
                                    ...
                                    className="mx-2 my-1 px-3 py-1.5 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-slate-400 w-[calc(100%-16px)]"
                                  />
                                  <button onClick={() => { setFilters({ ...filters, category: '' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-xs text-slate-500 hover:bg-slate-50">Clear Filter</button>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      {activeTab === 'tasks' && (
                        <th className="px-6 py-4 text-left bg-slate-50 !overflow-visible relative">
                          <div className="flex items-center justify-between group relative">
                            <span className="text-xs font-extrabold text-slate-900 uppercase tracking-wider">List</span>
                            <div className="relative">
                              <button
                              ...
                              {columnMenuOpen === 'list' && (
                                <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                  <div className="px-4 py-2 border-b border-slate-100 mb-1 flex items-center justify-between">
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Filter List</span>
                                    <button onClick={() => setColumnMenuOpen(null)} className="text-slate-400 hover:text-slate-600">
                                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                                    </button>
                                  </div>
                                  <input
                                    ...
                                    className="mx-2 my-1 px-3 py-1.5 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-slate-400 w-[calc(100%-16px)]"
                                  />
                                  <button onClick={() => { setFilters({ ...filters, list: '' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-xs text-slate-500 hover:bg-slate-50">Clear Filter</button>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      <th className="px-6 py-4 text-left bg-slate-50/50">
                        <div className="flex items-center justify-between group relative">
                          <button
                            ...
                            className="flex items-center gap-2 text-xs font-extrabold text-slate-900 uppercase tracking-wider hover:text-slate-700 transition-colors group"
                          >
                            {activeTab === 'meetings' ? 'Meeting' : 'Due Date'}
                            <svg className={`w-4 h-4 transition-all ${sortConfig?.column === (activeTab === 'meetings' ? 'meetingDate' : 'dueDate') ? 'text-slate-900' : 'text-slate-400 group-hover:text-slate-600'} ${sortConfig?.column === (activeTab === 'meetings' ? 'meetingDate' : 'dueDate') && sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 11l5-5m0 0l5 5m-5-5v12" />
                            </svg>
                          </button>
                          <div className="relative">
                            ...
                            {(columnMenuOpen === 'meeting' || columnMenuOpen === 'dueDate') && (
                              <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                ...
                                <button onClick={() => { setSortConfig({ column: activeTab === 'meetings' ? 'meetingDate' : 'dueDate', direction: 'asc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 text-slate-600">Sort by ASC</button>
                                <button onClick={() => { setSortConfig({ column: activeTab === 'meetings' ? 'meetingDate' : 'dueDate', direction: 'desc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 text-slate-600">Sort by DESC</button>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <>
                          <th className="px-6 py-4 text-left bg-slate-50 !overflow-visible relative">
                            <div className="flex items-center justify-between group relative">
                              ...
                              >
                                Assignee
                                <svg className={`w-4 h-4 transition-all ${sortConfig?.column === 'assignee' ? 'text-slate-900' : 'text-slate-400 group-hover:text-slate-600'} ${sortConfig?.column === 'assignee' && sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 11l5-5m0 0l5 5m-5-5v12" />
                                </svg>
                              </button>
                              <div className="relative">
                                ...
                                {columnMenuOpen === 'assignee' && (
                                  <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                    <div className="px-4 py-2 border-b border-slate-100 mb-1 flex items-center justify-between">
                                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Filter Assignee</span>
                                      <button onClick={() => setColumnMenuOpen(null)} className="text-slate-400 hover:text-slate-600">
                                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                                      </button>
                                    </div>
                                    <input
                                      ...
                                      className="mx-2 my-1 px-3 py-1.5 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-slate-400 w-[calc(100%-16px)]"
                                    />
                                    <button onClick={() => { setFilters({ ...filters, assignee: '' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-xs text-slate-500 hover:bg-slate-50">Clear Filter</button>
                                  </div>
                                )}
                              </div>
                            </div>
                          ...
                          <th className="px-6 py-4 text-left bg-slate-50 !overflow-visible relative">
                            <div className="flex items-center justify-between group relative">
                              ...
                              >
                                Deadline
                                <svg className={`w-4 h-4 transition-all ${sortConfig?.column === 'deadline' ? 'text-slate-900' : 'text-slate-400 group-hover:text-slate-600'} ${sortConfig?.column === 'deadline' && sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 11l5-5m0 0l5 5m-5-5v12" />
                                </svg>
                              </button>
                              <div className="relative">
                                ...
                                {columnMenuOpen === 'deadline' && (
                                  <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                    ...
                                    <button onClick={() => { setSortConfig({ column: 'deadline', direction: 'asc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 text-slate-600">Sort by ASC</button>
                                    <button onClick={() => { setSortConfig({ column: 'deadline', direction: 'desc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2 text-slate-600">Sort by DESC</button>
                                  </div>
                                )}
                              </div>
                            </div>
                        ...
                        <>
                          <th className="px-6 py-4 text-left bg-slate-50 !overflow-visible relative">
                            <div className="flex items-center justify-between group relative">
                              ...
                              >
                                Priority
                                <svg className={`w-4 h-4 transition-all ${sortConfig?.column === 'priority' ? 'text-slate-900' : 'text-slate-400 group-hover:text-slate-600'} ${sortConfig?.column === 'priority' && sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 11l5-5m0 0l5 5m-5-5v12" />
                                </svg>
                              </button>
                              <div className="relative">
                                ...
                                {columnMenuOpen === 'priority' && (
                                  <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                    <div className="px-4 py-2 border-b border-slate-100 mb-1 flex items-center justify-between">
                                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Filter Priority</span>
                                      <button onClick={() => setColumnMenuOpen(null)} className="text-slate-400 hover:text-slate-600">
                                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                                      </button>
                                    </div>
                                    <button onClick={() => { setFilters({ ...filters, priority: '' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 ${filters.priority === '' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>All</button>
                                    <button onClick={() => { setFilters({ ...filters, priority: 'High' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 ${filters.priority === 'High' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>High</button>
                                    <button onClick={() => { setFilters({ ...filters, priority: 'Medium' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 ${filters.priority === 'Medium' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>Medium</button>
                                    <button onClick={() => { setFilters({ ...filters, priority: 'Low' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 ${filters.priority === 'Low' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>Low</button>
                                  </div>
                                )}
                              </div>
                            </div>
                          ...
                          <th className="px-6 py-4 text-left bg-slate-50 !overflow-visible relative">
                            <div className="flex items-center justify-between group relative">
                              ...
                              >
                                Status
                                <svg className={`w-4 h-4 transition-all ${sortConfig?.column === 'status' ? 'text-slate-900' : 'text-slate-400 group-hover:text-slate-600'} ${sortConfig?.column === 'status' && sortConfig.direction === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 11l5-5m0 0l5 5m-5-5v12" />
                                </svg>
                              </button>
                              <div className="relative">
                                ...
                                {columnMenuOpen === 'taskStatus' && (
                                  <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-[100] min-w-[260px]">
                                    ...
                                    <button onClick={() => { setFilters({ ...filters, status: '' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 ${filters.status === '' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>All</button>
                                    <button onClick={() => { setFilters({ ...filters, status: 'completed' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 ${filters.status === 'completed' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>Completed</button>
                                    <button onClick={() => { setFilters({ ...filters, status: 'pending' }); setColumnMenuOpen(null); }} className={`w-full px-4 py-2 text-left text-sm hover:bg-slate-50 ${filters.status === 'pending' ? 'text-slate-900 font-bold bg-slate-50' : 'text-slate-600'}`}>Active</button>
                                  </div>
                                )}
                              </div>
                            </div>
                  ...
                  {/* Table Body */}
                  <tbody className="bg-white divide-y divide-slate-100 !overflow-visible">
                    {activeTab === 'meetings' ? (
                      paginatedData.map((item: any, idx: number) => (
                        <tr
                          key={`${item.id}-${idx}`}
                          className="hover:bg-gradient-to-r hover:from-slate-50 hover:to-white transition-all duration-200 group border-b border-slate-100"
                        >
                          {(activeReport === 'lessons' || activeReport === 'decisions') && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              {item.isCompleted ? (
                                <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-green-100 text-green-800 rounded-lg text-xs font-semibold">
                                ...
                                <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-medium">
                          ...
                          <td className="px-6 py-4">
                            <div className="max-w-md">
                              <p className="text-sm font-semibold text-slate-900 line-clamp-2">{item.text}</p>
                            </div>
                          </td>
                          ...
                          {activeReport === 'information' && (
                            <td className="px-6 py-4 whitespace-nowrap">
                              {item.category && (
                                <span className="inline-block px-3 py-1 bg-blue-100 text-blue-800 rounded-lg text-xs font-semibold">
                                  {item.category}
                                </span>
                              )}
                            </td>
                      ...
                      paginatedData.map((task: any) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              {task.list?.title ? (
                                <span className="text-sm text-slate-700">{task.list.title}</span>
                              ) : (
                                <span className="text-sm text-slate-400">—</span>
                              )}
                            </td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              {task.priority ? (
                                <span className={`inline-block px-3 py-1 rounded-lg text-xs font-semibold ${task.priority.toLowerCase() === 'high' ? 'bg-red-100 text-red-800' :
              ...
              <thead className="bg-slate-50 border-b-2 border-slate-200">
                <tr>
                  {activeTab === 'meetings' && (activeReport === 'lessons' || activeReport === 'decisions') && (
                    <th className="text-left px-6 py-3">
                      <div className="flex items-center justify-between group relative">
                        ...
                        {columnMenuOpen === 'status' && (
                          <div className="absolute top-full mt-1 bg-white rounded-lg shadow-xl border border-slate-200 py-1 z-50 min-w-[180px]">
                            <button
                  ...
                  {activeTab === 'tasks' && (
                    <th className="text-left px-6 py-3">
                      <div className="flex items-center justify-between group relative">
                        <span className="text-xs font-black text-slate-700 uppercase tracking-wider">List</span>
                        <button
                          onClick={() => toggleColumnMenu('list')}
                          className="p-1 hover:bg-slate-200 rounded text-slate-400 hover:text-slate-700 transition-colors"
                        >
                        ...
                        {columnMenuOpen === 'list' && (
                          <div className="absolute top-full mt-1 bg-white rounded-lg shadow-xl border border-slate-200 py-1 z-50 min-w-[180px]">
                            <button onClick={() => { setSortConfig({ column: 'list', direction: 'asc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>Sort by ASC</button>
                            <button onClick={() => { setSortConfig({ column: 'list', direction: 'desc' }); setColumnMenuOpen(null); }} className="w-full px-4 py-2 text-left text-sm hover:bg-slate-50 flex items-center gap-2"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>Sort by DESC</button>
                          </div>
                        )}
                      </div>
                    </th>
                  ...
                  {activeTab === 'meetings' && (activeReport === 'lessons' || activeReport === 'decisions') && (
                    <th className="text-left px-6 py-3">
                      <div className="flex items-center justify-between group relative">
                        <span className="text-xs font-black text-slate-700 uppercase tracking-wider">Deadline</span>
                        <button
                          onClick={() => toggleColumnMenu('deadline')}
              ...
              <tbody className="divide-y divide-slate-100">
                {currentData.length === 0 ? (
                  ...
                  paginatedData.map((task: any) => {
                    const isOverdue = task.dueDate && new Date(task.dueDate) < new Date() && !task.archived;
                    return (
                      <tr key={task.id} className="group transition-colors duration-200 border-b hover:bg-slate-50">
                        <td className="px-6 py-4 align-top">
            ...
            <div className="mt-6 bg-white px-6 py-4 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="text-sm text-slate-600">
                  ...
                  <span className="font-bold text-slate-900">{Math.min(endIndex, currentData.length)}</span> of{' '}
                  <span className="font-bold text-slate-900">{currentData.length}</span> results
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${currentPage === 1
                      ...
                      : 'bg-gradient-to-r from-slate-900 to-slate-800 text-white hover:shadow-lg transform hover:-translate-y-0.5'
                      }`}
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      return (
                        <button
                          ...
                          className={`w-10 h-10 rounded-xl font-bold text-sm transition-all ${currentPage === page
                            ? 'bg-gradient-to-r from-slate-900 to-slate-800 text-white shadow-lg transform scale-110'
